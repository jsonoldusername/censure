...
class CensorWords
{
	...
	public function __construct() {	
		$this->replacer = '*';
		$this->setDictionary(array('en-us', 'ass', 'cum', 'hell'));
	}
	...
	public function setDictionary($dictionary) {
		...
		$this->cumwords = $cumwords;
		
		$this->hellwords = $hellwords;
	...
	public function censorString($string) {
			...
			$asswords = $this->asswords;
			$cumwords = $this->cumwords;
			$hellwords = $this->hellwords;
			...
			for ($x=0; $x<count($badwords); $x++) {
                ...
                $tempCount = preg_match($badwords[$x], $string); // count number of swears
                if ($tempCount > 0){ //check if we have a swear
				    if (preg_match($badwords[$x], "ass") > 0){
				        for ($y=0; $y<count($asswords); $y++) {
				            $tempString = strtolower(preg_replace("/[^a-zA-Z 0-9]+/", "", $string));
                        if ($asswords[$y] == $tempString){
    				        $replacement[$x] = "ass";
    				        $tempCount -= 1;
    				        break;
                        }
				    ...
				    if (preg_match($badwords[$x], "cum") > 0 ){
				        for ($z=0; $z<count($cumwords); $z++) {
                    
    				     if ($cumwords[$z] == strtolower($string)){
    				        $replacement[$x] = "cum";
    				        $tempCount -= 1;
                        }
    			    }
				    }
    			    if (preg_match($badwords[$x], "hell") > 0 ){
				        for ($z=0; $z<count($hellwords); $z++) {
    				     if ($hellwords[$z] == strtolower($string)){
    				        $replacement[$x] = "hell";
    				        $tempCount -= 1;
                        }
    			    }
			...
			$newstring['count'] = $count;
			$newstring['clean'] =  preg_replace($badwords, $replacement, $newstring['orig'],-1,$count);
			//return $count;
			return $newstring;
